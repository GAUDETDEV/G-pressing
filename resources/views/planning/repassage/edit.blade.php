...
@section('title',"planning repassage")
...
<div class="container mt-3">
    ...
    <form action="{{route('postPlanningRepassage',['facture_repassage' => $facture_repassage])}}" method="POST" class="container shadow rounded-3" style="width:50%; margin-top:5%; padding: 5em; background-color: rgb(255, 255, 255);">
        <h1 class="modal-title fs-5" id="modalAjoutGerantLabel" style="color:rgb(6, 76, 125); font-weight: bold; border-bottom: solid 5px rgb(6, 76, 125); ">ATTRIBUTION DE TÂCHES (REPASSAGE) </h1>
        @method('post')
        @csrf
        ...
        <input type="date" class="form-control mt-3" name="fin_tache" value="{{$facture_repassage->date_retrait}}">
        @if ($errors)
        @error('fin_tache')
            <p style="color:rgb(217, 17, 17);">{{ $message }}</p>
        @enderror
        ...
        <select class="form-select" name="id_executant" id="">
            @forelse ($liste_repasseurs as $liste_repasseur)
            <option value="{{$liste_repasseur->id}}">{{$liste_repasseur->name}}</option>
            @empty
            <p style="color:rgb(217, 17, 17);">La liste des repasseurs est vide!</p>
            @endforelse
        </select>
