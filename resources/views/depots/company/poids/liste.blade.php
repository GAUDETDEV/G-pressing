...
<!--bouton d'ajout de packs-->
    ...
    <button type="button" class="btn" data-bs-toggle="modal" data-bs-target="#exampleModalAddPoids" style="background-color:rgb(66, 16, 250); color: white;">
        <i class="fa-solid fa-plus"></i> Ajouter en fontion du Poids
    </button>
...
<div class="row row-cols-1 row-cols-md-3 g-4 mt-4">
    ...
    @forelse ( $depot_edit_company_poids as $depot_edit_company_poid)
        <div class="col">
            <div class="card h-100 shadow">
                <div class="card-body">

                    <div class="card-text rounded" style="background-color:rgb(6, 76, 125);">
                        <p style=" color:white; font-weight: bold; border-radius: 5px; padding:7px;"><strong> Poids:</strong> <span style="font-size: 2em; color:rgb(166, 212, 245); font-weight: bold;">{{$depot_edit_company_poid->poids_vet}} Kilo(s)</span></p>
                        <p style=" color:white; font-weight: bold; border-radius: 5px; padding:7px;"><strong> Prix :</strong> <span style="font-size: 2em; color:rgb(166, 212, 245); font-weight: bold;">{{$depot_edit_company_poid->prix_depot}} Francs</span></p>
                    </div>

                </div>
                <div class="card-footer mt-5">

                <a href="{{route('modifyPoids',['depot_poids' => $depot_edit_company_poid->id])}}" type="button" class="btn mt-5" style="color:rgb(255, 255, 255); padding: 5px; background-color:rgb(26, 12, 87);"><i class="fa-solid fa-pen"></i> Modifier</a>
                <a href="{{route('deletePoids',['depot_poids' => $depot_edit_company_poid->id])}}" type="button" class="btn mt-5" style="color:rgb(255, 255, 255); padding: 5px; background-color:rgb(168, 92, 0);"><i class="fa-solid fa-trash"></i> Supprimer</a>
    ...
    <div class="modal fade" id="exampleModalAddPoids" tabindex="-1" aria-labelledby="exampleModalAddPoidsLabel" aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form action="{{route('postPoids')}}" method="POST">

                    @method('post')
                    @csrf

                    <label for="" style="color:rgb(58, 32, 60); font-weight: bold;">Le poids</label>
                    <input type="text" class="form-control mt-3" name="poids_vet" >
                    @if ($errors)
                    @error('poids_vet')
...
<div class="mt-5">
    {{$depot_edit_company_poids -> links()}}
</div>
