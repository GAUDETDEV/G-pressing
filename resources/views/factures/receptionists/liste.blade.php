    ...
    <table class="table table-striped shadow">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <form action="{{route('putEtatFacture',['facture' => $facture->id])}}" method="post">
                        @method('put')
                        @csrf
                        <input type="text" name="statut_facture" value="Régler" style="display: none">
                        <button type="submit" style="color:white; background-color:rgb(244, 74, 12); padding: 5px; border-radius: 10px; border: none;">{{ $facture->statut_facture }}</button>
                    </form>
                </td>
                @else
                <td>
                ...
                <td>
                    <a href="{{route("detailFactureReceptionist",["facture" => $facture->id])}}" type="button" class="btn" title="Détails" style='color: rgb(65, 11, 203);'><i class="fa-solid fa-eye"></i></a><!--boutton détails-->
                </td>
            </tr>
            @empty
            <tr>
