...
<div class="container mt-3">
    <form action="" method="get" action="{{route('listeFactureRepassage')}}" style="display: flex;">
...
<div class="mt-5">
    ...
    <table class="table table-striped shadow">
        <caption class="fs-5" style="color:rgb(15, 109, 177); border-top: solid 5px rgb(26, 12, 87);"><strong>Liste des factures en attente de repassage</strong></caption>
        <thead>
            <tr>
        ...
        <tbody>
            @forelse ($facture_repassages as $facture_repassage)
            <tr>
                <td>{{ $facture_repassage->id}}</td>
                <td>{{ $facture_repassage->nom_titulaire}}</td>
                <td>{{ $facture_repassage->tel_titulaire}}</td>
                <td>{{ $facture_repassage->etat_traitement}}</td>
                <td>{{ $facture_repassage->created_at}}</td>
                <td>{{ $facture_repassage->date_retrait}}</td>
                <td>{{ $facture_repassage->updated_at}}</td>
                <td>
                    <a href="{{route('planningRepassage',["facture_repassage" => $facture_repassage->id])}}" type="button" class="btn" title="Editer" style='color: rgb(255, 255, 255); background-color:rgb(4, 96, 216);'><i class="fa-solid fa-pen-to-square"></i></a><!--boutton éditer-->
                </td>
            </tr>
            @empty
            <tr>
            ...
            <tr>
                ...
                <td>
                    <a href="{{route('planningRepassage',["facture_repassage" => $facture_supplement->id])}}" type="button" class="btn" title="Editer" style='color: rgb(255, 255, 255); background-color:rgb(4, 96, 216);'><i class="fa-solid fa-pen-to-square"></i></a><!--boutton éditer-->
                </td>
            </tr>
            @empty
            <tr>
