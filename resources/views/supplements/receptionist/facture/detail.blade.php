...
<div class="mt-5">
    ...
    @if ($facture->statut_facture and $facture->etat_livraison == "Non")
        <a type="button" class="btn" href="{{route('receptionistGeneratePdfFactureSupplement',['facture' => $facture])}}" style='color: rgb(255, 255, 255); background-color: rgb(5, 5, 5); margin: 5px;'>Reçu en PDF</a>
    @elseif ($facture->statut_facture and $infos_livraison)
        <a type="button" class="btn" href="{{route('receptionistGeneratePdfFactureSupplement',['facture' => $facture])}}" style='color: rgb(255, 255, 255); background-color: rgb(5, 5, 5); margin: 5px;'>Reçu en PDF</a>
    @endif

    @if ($facture->etat_livraison == "Oui")
    <a type="button" class="btn" href="{{route('receptionistlisteLivraisonSupplement',["facture" => $facture->id])}}" style='color: rgb(255, 255, 255); background-color: rgb(15, 109, 177); margin: 5px;'>Renseigner les informations de livraison</a>
    @endif
    ...
    <ul class="list-group mt-2">
        <li class="list-group-item">
            ...
            <form action="{{route('receptionistEditDetailsReceptSupplement',["facture" => $facture->id])}}" method="POST">
    ...
    <ul class="list-group mt-2">
        <li class="list-group-item">
            ...
            <p><strong>Type de service</strong> {{$type_service->type_service}}</p>
    ...
    <table class="table table-striped shadow mt-2">
        ...
        <tbody>
            @forelse ($liste_recept_supplements as $liste_recept_supplement)
            <tr>
                <td>{{ $liste_recept_supplement->nom_vet}}</td>
                <td>{{ $liste_recept_supplement->color_vet}}</td>
                <td>{{ $liste_recept_supplement->caract_vet}}</td>
                <td>{{ $liste_recept_supplement->cat_vet}}</td>
                <td>{{ $liste_recept_supplement->quality_vet}}</td>
                <td>{{ $liste_recept_supplement->qte_vet}}</td>
                <td>{{ $liste_recept_supplement->prix_unitaire}} frs CFA</td>
                <td>{{ $liste_recept_supplement->prix}} frs CFA</td>
                <td>
                    <a href="{{route('receptionistModifyVetReceptSupplement',['vetement_supplement' => $liste_recept_supplement->id])}}" title="modifier" style='color: rgb(38, 11, 214);'><i class="fa-solid fa-pen"></i></a><!--boutton modifier-->
                </td>
